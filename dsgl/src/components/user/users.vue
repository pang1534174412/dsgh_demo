...
<!-- 面包屑 -->
    ...
    <el-card class="box-card">
        <!-- 搜索 -->
         <el-input placeholder="请输入内容" v-model="queryInfo.query" class="inputsearch" clearable @clear="getUserList">
...
export default {
        ...
        return {
            ...
            addFormRules: { 
                username: [
                    ...
                    {min:3,max:10,message:'用户名在3~5个字符之间',trigger:'blur'}
                ],  
                password: [
    ...
    methods: {
        ...
        async getUserList(){
            ...
            const res = await this.$http.get(`/api/users`,{params:this.queryInfo})
        //    console.log(res);
        ...
        async UserstateChange(userinfo){
          ...
          const res = await this.$http.put(`/api/users/${userinfo.id}/state/${userinfo.mg_state}`)
        ...
        editUser(){
            ...
            this.$refs.editFormRef.validate( async valid => {
                console.log(valid);
                if(!valid) return 
               const res = await this.$http.put(`/api/users/`+this.editForm.id,{email:this.editForm.email,mobile:this.editForm.mobile})
               if(res.meta.status !== 200){
