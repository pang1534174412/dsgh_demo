  ...
  <el-table
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        <el-popover trigger="hover" placement="top">
          <div slot="reference" class="name-wrapper">
            <el-tag size="medum">{{scope.row.pay_status==0?"未付款":"已付款"}}</el-tag>
          </div>
        </el-popover>
      </template>
    </el-table-column>
    ...
    width="380">
      <template slot-scope="scope">
        ...
        <el-button
            ...
            icon="el-icon-edit"
              size="mini"
              @click="amend(scope.$index, scope.row,amendVisible = true)"></el-button>
            <el-button
            icon="el-icon-location-outline"
              size="mini"
              type="success"
              @click="logistics(scope.$index, scope.row,dialogVisible = true)"
          >
        </el-button>
          ...
          <el-button
              type="warning"
              size="mini"
              @click="handleDelete(scope.$index, scope.row,detailsVisible=true)"
              >订单详情
              </el-button>
              <el-button
              type="danger"
              size="mini"
              @click="modifyorder(scope.$index, scope.row,modiVisible=true)"
               >修改订单状态
          </el-button>
              </template>
    </el-table-column>
      ...
      <el-dialog
        ...
        <el-dialog
            ...
            :before-close="handleClose">
            <div class="details">
              <div class="details_timer">下单时间：{{changetime(details.create_time)}}</div>
        ...
        <!-- 修改订单 -->
         ...
         <el-dialog
            ...
            <div class="goods_number">订单编号：{{modelgoods.order_number}}</div>
              <div class="modigoodsbox">
                 <div class="delivergoods">
                   订单是否发货：
                    <el-select v-model="value" @change="getconfirm" placeholder="请选择">
                    <el-option
               ...
               <div class="payprice">
                  <div class="payprice_left">订单价格：{{modelgoods.order_price}}</div>
                  <div>订单数量：1</div>
               </div>
               ...
               <div class="goods_status">
                 <el-form   label-width="70px">
                  ...
                  <el-radio-group v-model="modification.status_pay">
                      <el-radio :label="0" @change="getpay_status">未支付</el-radio>
                      <el-radio :label="1" @change="getpay_status">已付款</el-radio>  
                  </el-radio-group>
                  </el-form-item >
                  </el-form>
               </div>
            ...
            <span slot="footer" class="dialog-footer">
              <el-button :plain="true" @click="open1" >取 消</el-button>
              <el-button  :plain="true" type="primary" @click="goods_commit">确 定</el-button>
            </span>
        </el-dialog>
        </div>
    ...
    computed:{
     ...
     changelist(){
      ...
      let seconds=date.getSeconds();
       let timer=year+"-"+mounth+"-"+day+" "+hours+":"+minutes+":"+seconds;
       return timer
...
.updata{
  ...
  .goods_number{margin-bottom: 10px;}
  .modigoodsbox{
